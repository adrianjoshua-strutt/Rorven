...
- [ ] Finalize the package/module map for the first slice only.
- [ ] Define current domain entities for project, run, agent run, task, event, artifact metadata, and workspace binding.
...
- [ ] Add architecture checks for forbidden provider/framework imports and historical-schema branches in domain/application code.
- [ ] Define the first ports: `RunRepository`, `EventRepository`, `TaskQueue`, `AgentRuntime`, `ArtifactStore`, and `Clock`.
...
- [ ] Implement a local deterministic `AgentRuntime` adapter that creates one parent run and two child runs through platform-owned records.
