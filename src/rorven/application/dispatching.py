...
def orchestrator_dispatch_contract() -> str:
    ...
    return (
        ...
        "not require workspace evidence, CLI execution, or workspace edits.\n"
        "If the request requires listing files, reading files, checking workspace state, "
        "running a safe workspace command, or changing a workspace text file, dispatch an "
        "appropriate subagent instead of claiming the work is already done.\n"
        "If the current user asks to retry, try again, run that again, check again, "
        "or otherwise repeat a prior workspace/tool action, dispatch a new subagent "
        "for the repeated action. Do not answer from prior work-log facts as if the "
        "repeat already happened in the current run.\n"
        "Subagents run asynchronously and should complete their assigned workspace work "
        "using their tools before reporting back.\n"
        "Subagents can use brokered workspace.list_files, workspace.read_text_file, "
        "workspace.write_text_file, and workspace.run_shell_command tools. "
        "The write tool creates or overwrites scoped text files. The shell command tool "
        "is workspace scoped, audited, timeout-limited, and policy-denied for obvious destructive, "
